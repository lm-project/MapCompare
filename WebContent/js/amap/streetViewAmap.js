define(function(){
	var gdStreetView = {}, markers = [],lastDirect = 1;
	var AMaper = null,point = null,Sys = null,apanObj = null;
	/*
	 *改变小地图的的大小
	 */
	function  changeMinMapSize(em,oy){
		 var fy = em.pageY - oy;
		 var yPano = 350+fy,yAmap = 195-fy,b = 202-fy;
		 console.log("差值："+fy+"  街景的高度"+yPano +"  地图的高度："+yAmap);
		 if(yAmap<450&&yAmap>50){
			 $('#divAmapPano').css("height",yPano+"px");
			 $('#divAMap').css("height",yAmap+"px");
			 $('#amapStretchBnt').css("bottom",b+"px");
		 }
	}
	function showMinMap(){
		 $('#amapStretchBnt').css("bottom","202px");
		 $('#amapStretchBnt').css("background-position","-40px -386px");
		 $('#divAMap').css("height","195px");
		 $('#divAMap').css("border-top","");
		 $('#divAMap').css("display","block");
		 $('#divAmapPano').css("height","350px");
		 $('#divAmapPano').css("border-bottom","");
	...
	function showAmapPano(){
		 ...
		 $('#divAMap').css("display","none");
		 $('#amapStretchBnt').css("display","block");
		 $('#divAmapPano').css("display","block");
		 if(Sys.firefox){
			 $('#amapStretchBnt').css("background","red");	
		 }
		 ...
		 $('#hr').on("mousedown",function(e){
			 $(document).on("mouseup",function(){
				 $("#divAmapPano,#divAMap").off("mousemove");
				 $(document).off("mouseup");
			 });
			 $('#divAmapPano,#divAMap').on("mousemove",function(ev){
	...
	function streetViewComplete(result){
		if(result.info!="OK"||result.panoInfo==''){
			alert("sorry，高德地图此处还没有街景！");
			return;
		}
		showAmapPano();//		显示街景
	    apanObj = new AMap.Panorama('divAmapPano',{
	    		panoId:result.panoInfo[0].panoId,
	    		pov:{ heading:270, pitch:0 },
	    		zoom:5 
	    });
	    gdStreetView.apanObj = apanObj;
	    AMap.event.addListener(apanObj,'panochange',function(){
 	    	 var lastPoint = point;
	    	 console.log("之前的坐标："+ lastPoint);
	    	 var apapPosition = apanObj.getPosition();
	    	 console.log("之后的坐标：" + apapPosition);
	    	 point = apapPosition == null ? point : apapPosition;
	    	 AMaper.amap.setCenter(point);
	    	 addMarkerWhenChangePov( lastPoint );
	    });
	    AMap.event.addListener(apanObj,'povchange',	function(){
	    	addMarkerWhenChangePov(point);
	    });
	    if($("#curLinkText").text()==="关闭联动"){
		  	AMaper.curLinkAmap.addStreetViewLinkAMap();      
		}
	}
	gdStreetView.SearchStreetViewAMap = function(Amaper,points,sys){
		AMaper = Amaper;
		point = points;
		Sys = sys;
		lastDirect = 1;
		var panoramaService = new AMap.PanoramaService();    //实例化全景查询服务AMaper.curLinkAmap
		panoramaService.getPano(point,500); 				 //根据坐标和半径查询邻近街景点信息
		AMap.event.addListener(panoramaService,'complete',function(result){
			streetViewComplete(result);
		});
	    AMap.event.addListener(panoramaService,'error', function(error){
		   alert("高德街景搜索失败！"+error.info);
		});
	};	
	return gdStreetView;
